...
class DashboardRepository {
  ...
  Future<List<AreaCount>> getTramitesPorArea() async {
    final res = await httpService.get('/reportes/tramites-por-area');
    final data = res.data;
    if (data is List) {
      return data.map((e) => AreaCount.fromJson(e as Map<String, dynamic>)).toList();
    }
    // If API returns object with data field
    if (data is Map && data['data'] is List) {
      return (data['data'] as List).map((e) => AreaCount.fromJson(e as Map<String, dynamic>)).toList();
    }
    return [];
  }
