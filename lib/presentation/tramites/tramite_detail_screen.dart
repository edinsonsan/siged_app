...
class _TramiteDetailScreenState extends State<TramiteDetailScreen> {
  ...
  Future<void> _fetchById(int id) async {
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Error al cargar trámite: ${e.toString()}')),
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return BlocProvider(
      create:
          (context) =>
              DetailTramiteCubit(context.read<TramiteRepository>())
                ..loadTramiteById(tramite.id),
      child: Scaffold(
        appBar: AppBar(title: Text('Trámite ${tramite.cut}')),
        body: Padding(
          ...
          child: BlocConsumer<DetailTramiteCubit, DetailTramiteState>(
            ...
            builder: (context, state) {
              if (state is DetailTramiteLoading ||
                  state is DetailTramiteInitial) {
          ...
          children: [
                  ...
                  children: [
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          Text(
                            'CUT ${state.tramite.cut}',
                            style: Theme.of(context).textTheme.headlineSmall
                                ?.copyWith(fontWeight: FontWeight.bold),
                          ),
                          const SizedBox(height: 6),
                          Text(
                            state.tramite.asunto,
                          ...
                          Row(
                            children: [
                              ...
                              const SizedBox(width: 6),
                              Text(
                                state.tramite.areaActual?.nombre ?? 'N/A',
                                style: Theme.of(context).textTheme.bodyMedium,
                              ),
                            ],
                          ),
                    ...
                    Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 8),
                        Text(
                          'Creado: ${df.format(state.tramite.fechaCreacion)}',
                ...
                child: Column(
                  children:
                      state.historial.map((h) {
                        ...
                        final parsedDate =
                            DateTime.tryParse(h.fecha) ?? DateTime.now();
                        return TimelineTile(
                          ...
                          indicatorStyle: IndicatorStyle(
                            ...
                            color:
                                isLast
                                    ? statusColor(state.tramite.estado)
                          ...
                          endChild: Padding(
                            ...
                            child: Column(
                              ...
                              children: [
                                Text(
                                  '${h.usuario != null ? '${h.usuario!.nombre} ${h.usuario!.apellido}' : 'Sistema'} • ${DateFormat.yMMMd().add_jm().format(parsedDate)}',
                                  style: const TextStyle(
            ...
            Card(
              ...
              child: Padding(
                padding: const EdgeInsets.all(8.0),
                child:
                    state.tramite.documentosAdjuntos.isEmpty
                        ? Padding(
                        ...
                        : Column(
                          children:
                              state.tramite.documentosAdjuntos
                                  .map(
                                    (doc) => _attachmentItem(
                                      doc: doc,
                                      onOpen: () => _launchUrl(doc.urlArchivo),
                                    ),
                                  )
                                  .toList(),
                        ),
            ...
            Builder(
              builder: (context) {
                ...
                final userRole = authState.user.rol;

                final canDerivar =
                    userRole == UserRole.ADMIN ||
                    userRole == UserRole.AREA ||
                    userRole == UserRole.MESA_PARTES;
                final canFinalizarYObservar =
                    userRole == UserRole.ADMIN || userRole == UserRole.AREA;
                ...
                return Row(
                  children: [
                    if (canDerivar)
                      ElevatedButton.icon(
                        onPressed:
                            () => _showDerivarDialog(context, state.tramite),
                        icon: const Icon(Icons.send),
                        label: const Text('Derivar'),
                      ),
                    if (canDerivar) const SizedBox(width: 8),

                    if (canFinalizarYObservar)
                      ElevatedButton.icon(
                        onPressed:
                            () => _confirmFinalizar(context, state.tramite),
                        icon: const Icon(Icons.check_circle),
                        label: const Text('Finalizar'),
                      ),
                    if (canFinalizarYObservar) const SizedBox(width: 8),

                    if (canFinalizarYObservar)
                      TextButton.icon(
                        onPressed:
                            () => _showObservarDialog(context, state.tramite),
  ...
  Widget _attachmentItem({required dynamic doc, required VoidCallback onOpen}) {
    ...
    final uploaded =
        doc.fechaSubida != null
            ? DateTime.tryParse(doc.fechaSubida.toString())
            : null;
    final subtitle =
        uploaded != null ? DateFormat.yMMMd().format(uploaded) : null;
    final isPdf = (doc.tipo ?? '').toString().toLowerCase().contains('pdf');
    return ListTile(
      leading: CircleAvatar(
        backgroundColor: isPdf ? Colors.red.shade100 : Colors.blue.shade50,
        child: Icon(
                    ...
                    ElevatedButton(
                      onPressed: () {
                        if (selectedArea != null) {
                          ...
                          final authState =
                              dialogContext.read<AuthCubit>().state;
                          final int? userId =
                              authState is AuthAuthenticated
                                  ? authState.user.id.toInt()
                                  : null;
                          Navigator.of(context).pop();
                          dialogContext.read<DetailTramiteCubit>().derivar(
                            tramite.id.toInt(),
                            toAreaId,
                            userId: userId,
                          );
                ...
                onPressed: () {
                  ...
                  if (comment.isNotEmpty) {
                    ...
                    if (authState is! AuthAuthenticated) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                          content: Text('Debe iniciar sesión para observar'),
                        ),
                      );
                      return;
                    }
                    final int userId = authState.user.id.toInt();
                    dialogContext.read<DetailTramiteCubit>().observar(
                      tramite.id.toInt(),
                      comment,
                      userId,
                    );
              ...
              ElevatedButton(
                onPressed: () {
                  ...
                  if (authState is! AuthAuthenticated) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(
                        content: Text('Debe iniciar sesión para finalizar'),
                      ),
                    );
                    return;
                  }
                  final int userId = authState.user.id.toInt();
                  dialogContext.read<DetailTramiteCubit>().finalizar(
                    tramite.id.toInt(),
                    userId,
                  );
                  Navigator.of(context).pop();
